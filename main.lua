...
task.spawn(function()
    ...
    ----------------------------------------------------------------
    -- ✅ API HELPERS (FIX: add since/wait/heartbeat)
    ----------------------------------------------------------------
    local sinceTs = 0
    ...
    local filterBtn = mkNeonGlowButton(midHeader, "🔍 Filter", 26, THEME.cyan)
    ...
    local autoJoinSkipExisting = false -- ✅ bật AutoJoin: bỏ qua item cũ, chờ item mới

    ----------------------------------------------------------------
    -- ✅ AUTO JOIN SPAM (ONLY AUTO JOIN, NOT TOUCH MANUAL SPAM JOIN)
    -- - đang auto spam mà xuất hiện 🌈 mới -> switch qua 🌈 luôn
    ----------------------------------------------------------------
    ...
    local function startAutoSpam(jid, iconType, statusMsg)
        ...
        if statusMsg and statusMsg ~= "" then
            ...
            createPopup(statusMsg, (iconType == "rainbow" and "🌈" or "🚀"), 2)
        end

        task.spawn(function()
            while autoSpam.running and autoSpam.jobid == jid and autoJoinEnabled do
                if badJobIds[jid] then
                    stopAutoSpam("⛔ AutoJoin: Job bị blacklist -> stop")
                    break
                end
                ...
                if game.JobId == jid then
                    stopAutoSpam("🚀 AutoJoin: Joined " .. jid)
                    createPopup("AutoJoin Joined: "..jid, "🚀", 2)
                    break
                end
                ...
                if not ok then
                    ...
                    if low:find("restricted", 1, true) or low:find("773", 1, true) then
                        stopAutoSpam("⛔ AutoJoin: Restricted -> stop")
                        createPopup("Restricted -> stop auto: "..jid, "⛔", 3)
                        break
                    end
                end
    ...
    ----------------------------------------------------------------
    -- SPAM JOIN (ONLY WHEN CLICK)  ✅ giữ nguyên
    ----------------------------------------------------------------
    ...
    ----------------------------------------------------------------
    -- ✅ ADD/UPDATE ROW (Sort: Rainbow > Diamond > Money)
    ----------------------------------------------------------------
    local function addOrUpdateRow(data)
        ...
        local iconPrefix = ""
        if data.iconType == "rainbow" then iconPrefix = "🌈 "
        elseif data.iconType == "diamond" then iconPrefix = "💎 " end
        ...
        joinBtn.MouseButton1Click:Connect(function()
            local ok, msg = joinJob(jid)
            if ok then
                setStatusText("✅ Joining: " .. jid, THEME.green)
                createPopup("Teleport: " .. jid, "✅", 2)
                stopSpam(jid)
                setSpamVisual(false)
            else
                setStatusText("❌ Join failed: " .. tostring(msg), THEME.red)
                createPopup("Join failed: " .. tostring(msg), "❌", 3)
            end
        end)
        ...
        spamBtn.MouseButton1Click:Connect(function()
            ...
            if st and st.running then
                stopSpam(jid)
                setSpamVisual(false)
                createPopup("Stopped spam: "..jid, "⛔", 2)
                return
            end

            spamJoinState[jid] = { running = true }
            setSpamVisual(true)
            createPopup("Spamming join: "..jid, "⚡", 2)
            ...
            task.spawn(function()
                while spamJoinState[jid] and spamJoinState[jid].running do
                    ...
                    if game.JobId == tostring(jid) then
                        stopSpam(jid)
                        setSpamVisual(false)
                        setStatusText("🚀 Joined: " .. jid, THEME.green)
                        createPopup("Joined: "..jid, "🚀", 2)
                        break
                    end
                    ...
                    if not ok then
                        ...
                        if low:find("restricted", 1, true) or low:find("773", 1, true) then
                            stopSpam(jid)
                            setSpamVisual(false)
                            setStatusText("⛔ Restricted -> stop spam: " .. jid, THEME.red)
                            createPopup("Restricted -> stop spam: "..jid, "⛔", 3)
                            break
                        else
                            setStatusText("⚠️ Spam trying... "..tostring(msg), THEME.muted)
                        end
                    end
    ...
    do
        ...
        local tabA = mkButton(tabs, "💎🌈 Diamond & Rainbow", 34, THEME.cyan)
        ...
        mkChip("__BLOCK_DIAMOND__", "💎 Block Diamond", "DiamondRainbow", "icon")
        mkChip("__BLOCK_RAINBOW__", "🌈 Block Rainbow", "DiamondRainbow", "icon")
        ...
        if canFileIO() then
            createPopup("✅ Blacklist Saved: "..SAVE_FILE, "💾", 3)
        else
            createPopup("⚠️ Executor không có readfile/writefile -> blacklist KHÔNG lưu", "⚠️", 4)
        end
    end
    ...
    local function setAutoJoin(on)
        ...
        if on then
            ...
            setStatusText("⏳ Auto Join: ON (chờ item mới)", THEME.green)
            createPopup("Auto Join ON: SẼ CHỜ ITEM MỚI (không join item cũ)", "⏳", 2)
        else
            stopAutoSpam("Auto Join: OFF")
            setStatusText("Auto Join: OFF", THEME.muted)
            createPopup("Auto Join Disabled", "⏸️", 2)
        end
    end
    ...
    local function setTier(t)
        ...
        if t then
            setStatusText("Filter: ≥ " .. t, THEME.muted)
        end
    end
    ...
    local function applyMinFromBox()
        ...
        if val > 0 then
            ...
            tierSelected = nil
            setStatusText("Filter: ≥ " .. formatMoney(val), THEME.muted)
        end
    end
            ...
            for jid, st in pairs(retryState) do
                ...
                elseif now >= (st.next or 0) then
                    ...
                    if ok then
                        ...
                        setStatusText("🚀 Retry OK: " .. jid, THEME.green)
                        createPopup("Retry OK: "..jid, "🚀", 2)
                    else
                        if tostring(msg):lower():find("blacklisted", 1, true) then
                            retryState[jid] = nil
                        else
                            setStatusText("⏳ Retry failed: "..tostring(msg), THEME.red)
                            scheduleRetry(jid)
                        end
                    end
                end
    ...
    task.spawn(function()
        ...
        if not API_BASE or API_BASE == "" then
            setStatusText("❌ Missing API_BASE", THEME.red)
            createPopup("Bạn chưa điền API_BASE", "❌", 4)
            return
        end

        setStatusText("🔄 Checking API /health ...", THEME.muted)

        local okHealth, errHealth = apiHealth()
        if not okHealth then
            setStatusText("❌ API offline: " .. tostring(errHealth), THEME.red)
            createPopup("API offline: "..tostring(errHealth), "❌", 4)
            return
        end

        setStatusText("✅ API Connected - Polling /items/simple", THEME.green)
        createPopup("API Connected", "✅", 2)
        ...
        while task.wait(POLL_INTERVAL) do
            ...
            if not data then
                if tick() - lastGood > 2.0 then
                    setStatusText("❌ /items/simple error: " .. tostring(err), THEME.red)
                end
                continue
            end
            ...
            if data.ok ~= true then
                if tick() - lastGood > 2.0 then
                    setStatusText("❌ /items/simple not ok", THEME.red)
                end
                continue
            end
            ...
            setStatusText("🟢 Live (API): " .. tostring(#items) .. " items", THEME.muted)
            ...
            if autoJoinEnabled and autoJoinSkipExisting then
                ...
                setStatusText("⏳ AutoJoin armed: đang chờ item MỚI...", THEME.muted)
            else
                -- ✅ AUTO JOIN SWITCH LOGIC: đang auto spam mà có 🌈 mới -> nhảy qua 🌈 luôn
                if autoJoinEnabled and #newCandidates > 0 then
                    ...
                    if bestCandidate and bestCandidate.jobid then
                        ...
                        -- nếu đang auto spam và có 🌈 mới -> switch ngay
                        if autoSpam.running and autoSpam.jobid and autoSpam.jobid ~= jid then
                            if isRainbow and autoSpam.iconType ~= "rainbow" then
                                stopAutoSpam("🌈 Rainbow mới xuất hiện -> chuyển mục tiêu")
                                local msg = "🌈 AutoJoin SWITCH -> " .. jid .. " ("..formatMoney(bestCandidate.money)..")"
                        ...
                        -- nếu chưa auto spam gì thì start
                        if not autoSpam.running then
                            local tag = isRainbow and "🌈" or "🚀"
